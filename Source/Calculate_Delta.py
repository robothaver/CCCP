...
class CalculateDelta:
    ...
    def check_end_time(self):
        ...
        for x, start_time in enumerate(self.config.class_start):
            if datetime.strptime(start_time, "%H:%M:%S") <= self.current_time <= \
                    datetime.strptime(self.config.class_end[x], "%H:%M:%S"):
                # If class is now
                self.is_lesson_over = False
                self.end_time = datetime.strptime(self.config.class_end[x], "%H:%M:%S")
            if datetime.strptime(start_time, "%H:%M:%S") <= self.current_time:
    ...
    def break_time_difference(self):
        ...
        if not self.class_number == 8 or 0:
            ...
            break_time_difference_var = \
                datetime.strptime(self.config.class_start[self.class_number], "%H:%M:%S") - self.current_time
